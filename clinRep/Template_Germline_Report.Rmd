...
RunCV=ifelse(is.null(data$CNVFull), F, T)
EvalCNV=F
if (RunCV){
 T1=read.delim(data$CNVFull, sep="\t")
 EvalCNV=ifelse(nrow(T1)>0, T, F)
}
```
...
RunSV=ifelse(is.null(data$SVFull), F, T)
EvalSV=F
if (RunSV){
 S1=read.delim(data$SVFull, sep="\t")
 EvalSV=ifelse(nrow(S1)>0, T, F)
}
```

`r ifelse(RunSV==F, print('SV data not available for this sample'), print("")) `
`r ifelse(EvalSV==F, print('Analysis did not return any SVs'), print(""))`
...
`r ifelse(CallPloidy==T, sprintf('The sex of the patient is %s', isX))`

`r ifelse(CallPloidy==T & length(whichDiff)==0, print('All autosomes have normal ploidy.'), print(paste("Chromosome", unlist(whichDiff), "has aberrant ploidy", collapse=" ")))`
