...
```{r, eval=T}
RunCV=ifelse(is.null(data$cnv), F, T)
```

`r ifelse(RunCV==F, print('CNV analysis not available this sample'), print("")) `


```{r child = 'gatk-cnv-insert.Rmd', eval=RunCV}
```
...
```{r}
RunSV=ifelse(is.null(data$Manta), F, T)
```

`r ifelse(RunSV==F, print('SV analysis not available this sample'), print("")) `

```{r child = 'sv-table.Rmd', eval=RunSV}
```
...
Variants have been grouped into 4 main clusters:

*Group A: Cancer Related* These genes have been documented to be oncogenic or tumour suppressors
*Group B: Pathway Related* Variants in genes that affect pathways which are related to the mode of action of the treatment
*Group C: Protective or drug related* These variants have been documented to have either a protective effect, or can influence the efficacy of drug treatments
*Group D: Variants of Unknown Significance* These variants are predicted to be highly deleterious but do not appear in any of the previous categories

#### CNVs

A panel of normals limited to Tempus probed regions was used in this analysis

Definitions: (assuming normal is diploid)

  * Hom Loss: log2FC less than (-2) 
  * Het Loss: log2FC less than (-1) (i.e. CN:1 in Tumor)
  * Gain: log2FC >0.58 (i,e. CN:3 in Tumor)
  * Amp: log2FC > 1 (i.e. CN: 4 +)
  
Only Cancer related genes (from COSMIC) are listed
