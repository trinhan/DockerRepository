...
CreateGRangesData=function(ASV, Col, SVmode, Ncheck=500){
  if (SVmode){
    ...
    gainMat <- GRanges(seqnames=paste0("chr",ASV[, "CHROM"]), ranges=IRanges(start=ASV[, "START"], end=ASV[, "END"]), 
                       path=ASV$PathogenicSource, AF=ifelse(is.na(ASV$Pop.Freq), 1, 1-ASV$Pop.Freq))
  }
