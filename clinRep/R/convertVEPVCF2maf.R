...
convertVCFtoMAF <- function(vcffile, outputfile, sampleName, canonical = TRUE, runMode = "Tumour", AAlist = NULL, minCallers = NULL) {
  ...
  for (i in 1:(length(Ntries) - 1)) {
    ann <- extract.info(inputFile[(Ntries[i] + 1):Ntries[i + 1]], element = 'CSQ', as.numeric = FALSE)
...
# Main function
main <- function() {
  convertVCFtoMAF(opts$vcffile, opts$outputfile, opts$sampleName, opts$canonical, opts$runMode, opts$AAlist, opts$minCallers)
}
