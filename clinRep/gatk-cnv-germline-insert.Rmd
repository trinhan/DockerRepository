...
if (data$`Analysis Mode`=="Tumour"){ 
  temp1$CN=cut(temp1$CN, breaks=c(-1, 1,  3, 6, 10, 30, 100))
  fillvals=c("#3182bd", "white", brewer.pal(4, "Reds"))
}else{
  temp2$CN=factor(temp2$CN, levels=c(0, 1, 3, 4))
  fillvals=c("#3182bd", "#9ecae1", "#fc9272","#de2d26", "white")
}
...
ggplot(temp2, aes(x=Gene_name, y=1, fill=CN))+geom_tile()+theme(axis.text.x = element_blank(), axis.ticks.x = element_blank())+scale_fill_manual(values = fillvals)+expand_limits(y = 0.25)+geom_text(aes(colour = colB, label = Gene_name), y = 0.35, angle=90)+facet_grid(~SV_chrom, space="free", scales="free")+scale_colour_manual(values = c("blue", "purple", "red"), guide = F)
```
