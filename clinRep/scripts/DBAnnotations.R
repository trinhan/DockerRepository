...
appendMSigDB<-function(AllData, MSigDB, colcat="h"){
  PathInH=getGmt(con=MSigDB, geneIdType=SymbolIdentifier(), collectionType=BroadCollection(category="h"))
  Mx2=geneIds(PathInH)
  Ids2=names(PathInH)
...
appendCosmicMut<-function(AllData, cosmicMut){
...
appendCosmicGeneCensus<-function(AllData, cosmicGenes){
  Cosmic2=read.csv(cosmicGenes)
...
appendProteinInfo<-function(AllData, pfam, pirsf){
  # a. read in data
...
convertMAFtoAnnotations <- function(maffile, outputfile, sampleName = NULL, cosmicMut, cosmicGenes, MSigDB, pfam = NULL, pirsf = NULL) {
  
#####################################################################
#1. Read in the relevant headers in the output maf file to save space
#####################################################################  
AllData=fread(maffile, sep="\t", stringsAsFactors = F, select=c("Hugo_Symbol","CHROM","POS", "REF", "ALT", "HGVSp_Short","HGVSc", "DOMAINS"))
...
AllData<-appendMSigDB(AllData, MSigDB, "h")
...
AllData<-appendCosmicMut(AllData, cosmicMut)
...
AllData<-appendCosmicGeneCensus(AllData, cosmicGenes)

#########################################
#6. Annotate with pfam information
#########################################

print('Add protein domain annotations from pfam and psird')
AllData<-appendProteinInfo(AllData, pfam, pirsf)
...
convertMAFtoAnnotations(maffile=opts$maffile, outputfile=opts$outputfile, cosmicMut=opts$cosmicMut, 
                        cosmicGenes=opts$cosmicGenes, MSigDB=opts$MSigDB, pfam=opts$pfam, pirsf=opts$pirsf)
