...
* ACMG score 3 and above
* Variant Allele Frequency above 0.25
...
CreateGRangesData=function(AnnotT, Col, Ncheck){
  ...
  gMat$AFcol=brewer.pal(5, Col)[cut(gMat$AF, c(0, 0.5, 0.8, 0.9, 0.95, 1))]
...
kpPlotMarkers(kp, data=genes, labels=genes$hgnc_symbol, text.orientation = "horizontal",
              r1=0.75, cex=0.5, adjust.label.position = FALSE)
```
...
```{r}
lx1=which((AnnotSV$GenesOfInterest!=""| AnnotSV$Cosmic!="") & AnnotSV$ACMG_class>=3 & (AnnotSV$AF<=0.1 | is.na(AnnotSV$AF)) & AnnotSV$MeanVAF>0.25)
Scolnames=c("SV_chrom", "SV_start", "SV_end","SV_type", "SV_length", "ACMG_class", "AF", "GT","MeanVAF",
            "GenesOfInterest","Cosmic", "Pathways","GTex", "Gene_name",  "Pheno")
T1=AnnotSV[lx1, Scolnames]
colnames(T1)=c("CHROM", "START", "END", "TYPE", "SV_length", "ACMG", "Pop Freq", "GT", "VAF", "GenesOfInterest", "Cosmic", "Pathways", "GTex", "Gene Names", "CLinVar Phenotypes")

DT::datatable(T1, rownames=F, filter = 'top', class='cell-border stripe',extensions="Buttons", options=list(dom="Bfrtip",scoller = TRUE, pageLength = 5,  buttons=c(I('colvis'),'csv', 'excel'), columnDefs = list(list(visible=FALSE, targets=c(10:13)))))
```
...
* pLI: intolerance to loss of function predictions on a gene level from gnomad or ExAC
...
```{r}
CGenes=which(AnnotSVGenes$Cosmic!="" & AnnotSVGenes$MeanVAF>0.2)
...
Scolnames=c("Gene_name","AnnotSV_ID", "SV_type", "SV_length", "ACMG_class", "Location", "AF", "HI", "TS","OMIM_phenotype", "OMIM_morb",
            "RE_gene", "Dist_nearest_SS", "TAD_coordinate",
             "Bsource", "Psource", "GnomAD_pLI", "ExAC_pLI",
            "MeanVAF", "Depth")
CGenesSVfilt=CGenesSV[ ,Scolnames]

colnames(CGenesSVfilt)=c("GENE","SV_ID", "TYPE", "SV_length", "ACMG", "Location", "Pop Freq", "Haploinsufficiency", "Triplosensitivity", "OMIM", "OMIM_morbid", "Reg.Element.Genes", "Nearest.Splice.Site.bp", "TAD", "BenignSource", "PathogenicSource", "gnomad.pLI", "exac.pLI", "MeanVAF", "Depth")

DT::datatable(CGenesSVfilt, rownames=F, filter = 'top', class='cell-border stripe',extensions="Buttons", options=list(dom="Bfrtip",scoller = TRUE, pageLength = 7,  buttons=c(I('colvis'),'csv', 'excel'), columnDefs = list(list(visible=FALSE, targets=c(9, 11, 12, 13,14, 17:19)))))
...
```{r}
PGenes=which(AnnotSVGenes$GenesOfInterest!="" & AnnotSVGenes$MeanVAF>0.2)
...
colnames(PGenesSVfilt)=c("GENE","SV_ID", "TYPE", "SV_length", "ACMG", "Location", "Pop Freq", "Haploinsufficiency", "Triplosensitivity", "OMIM", "OMIM_morbid", "Reg.Element.Genes", "Nearest.Splice.Site.bp", "TAD", "BenignSource", "PathogenicSource", "gnomad.pLI", "exac.pLI", "MeanVAF", "Depth")

DT::datatable(PGenesSVfilt, rownames=F, filter = 'top', class='cell-border stripe',extensions="Buttons", options=list(dom="Bfrtip",scoller = TRUE, pageLength = 7,  buttons=c(I('colvis'),'csv', 'excel'), columnDefs = list(list(visible=FALSE, targets=c(9, 11, 12, 13,14, 17:19)))))
