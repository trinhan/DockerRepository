#!/usr/bin/Rscript
"usage: \n SummarizeVariants.R [--maffile=<file> --outputname=<string> --germline=<boolean> --caddscore=<integer> --gnomadcutoff=<float> --exomesize=<float> --pathwayList=<string> ]
...
\n --pathwayList=<string> a List to return variants of a specific pathway (believed to be related to response)" -> doc
...
SummarizeVariants=function(maffile, outputname, germline=T, caddscore=20, gnomadcutoff=0.1, exomesize=45, pathwayList){
  ...
  InputData=fread(maffile, sep="\t", select=c("Hugo_Symbol", "CHROM","POS", "HGVSp_Short","HGVSc","EXON","Consequence","gnomAD_AF",
               ...
               "CMC.MUTATION_SIGNIFICANCE_TIER", "CMC.ONC_TSG", "CMC.DISEASE", "HallmarkPathways", gnames))
