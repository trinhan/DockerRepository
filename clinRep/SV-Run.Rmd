...
gainMat2=tryCatch({CreateGRangesData(AnnotSV[c(gain, ins), ], "Reds",T, 500)}, error=function(e){GRanges()})
lossMat2=tryCatch({CreateGRangesData(AnnotSV[homloss, ], "Blues",T, 500)}, error=function(e){ GRanges()})
invMat2=tryCatch({CreateGRangesData(AnnotSV[c(inv, bnd)], "Green",T, 500)}, error=function(e){GRanges(names=NULL, path=NULL, AF=NULL)})

if (length(S2$Gene_name)>1){
  genes=genesAll[na.omit(match(S2$Gene_name, genesAll$hgnc_symbol))]
}
...
```{r, eval=MGexist}
DT::datatable(S1[idx, ], rownames=F, filter = 'top', class='cell-border stripe',extensions="Buttons", options=list(dom="Bfrtip",scoller = TRUE, pageLength = 5,  buttons=c(I('colvis'),'csv', 'excel'), columnDefs = list(list(visible=FALSE, targets=c(12:14)))))
```
