...
def filter_mutations(FILE_IN,FILE_OUT_REJECTED, FILE_OUT_PASSED, passed_flag, VariantCaller):	
		...
		for line in in_file:
			if not(line.startswith('#')) and not(line.startswith('CONTIG')) and not(line.startswith('contig')):
				if VariantCaller!="None":
					pieces = line.strip('\n').split('\t')
					pieces[7] = pieces[7] + ';ME=' + str(VariantCaller)
					line='\t'.join(pieces) + '\n'			
...
if __name__ == '__main__':
	...
	if(args):
		...
		filter_mutations(args.FILE_IN, args.FILE_OUT_REJECTED, args.FILE_OUT_PASSED, args.passed_flag, args.VariantCaller)
	else:
		parser.print_help()%                                                                                                                                          a